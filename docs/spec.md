...
| T40 | 7 — SEO | Verify Coolblue-click tracking in GA — confirm the `click_deal` events fired from `DealCard` and the product detail page's `AffiliateLink` (added ad hoc during T33) actually show up correctly in GA4; decide the intended measurement design (is `click_deal` the right event name/shape for both deal-card and plain product-page clicks? should it distinguish deal vs. non-deal context? does GA4's own Enhanced Measurement outbound-click tracking already cover this, making the manual event redundant or conflicting?); also want a breakdown of which page/context (homepage, `/deals`, category page, product page) *and* which individual product drives the most outbound Coolblue clicks — `click_deal` already passes `product_name`/`deal_id` so per-product breakdown may already be possible, but the source page/context isn't currently captured as an event param and would need adding. Note: Coolblue affiliate approval is still pending — current links are not yet earning affiliate commission | ⬜ |

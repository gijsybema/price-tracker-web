...
| T35a | 8 — UX | Extend price-history display beyond 90 days. The backend's AI deal-description prompt currently reports "laagste prijs in de afgelopen 90 dagen" (not "ooit") specifically because the site's price-history graph only shows 90 days — so a longer true low can't be verified by the visitor. If the graph is extended to show more history, src/db.py's get_price_context (product_scraper repo) should be revisited to widen or drop the 90-day bound accordingly. | ⬜ |
| T35b | 8 — UX | Lowest-price-ever indicator on product detail page — query all-time low from price history, show "Laagste prijs ooit: €X" label; highlight when current price ≤ all-time low (or within threshold TBD) | ⬜ |
...
| T40 | 7 — SEO | Verify Coolblue-click tracking in GA — root cause of a reported "popup instead of new tab" bug traced to the onClick+target="_blank" pattern in DealCard/AffiliateLink triggering ad-blocker anti-redirect heuristics; removed manual click_deal gtag calls entirely (plain anchors now), confirmed GA4 Enhanced Measurement "Outbound clicks" already covers tracking with page_location (per-page breakdown) and link_url (per-product breakdown, at the cost of no friendly product name — raw Coolblue URLs only); decided against a custom event since only Coolblue links are outbound today. Follow-up for the user: register link_url as a custom dimension in GA4, mark `click` as the new Key Event, retire the stale click_deal Key Event | ✅ |
| T41 | 8 — UX | Check whether the AI search bar's mobile UI is optimized — review layout, sizing, and usability of the search input/results on small viewports | ⬜ |
